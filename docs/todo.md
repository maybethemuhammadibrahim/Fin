...
| 17 | ~~`.xlsx` is offered in the uploader's accepted types but only `.csv` is parsed.~~ **Removed from `ACTUALS_TYPES` 2026-08-19**, same fix as `.docx` got on 2026-08-17: stop advertising what nothing can read. `#38` itself stays open — spreadsheets are still unreadable, they are just no longer offered. Reading one needs a new dependency and therefore an ADR. `tests/test_file_uploader.py` now fails if any offered extension cannot be routed. | `#38` `#68` |
...
| 27 | **`csv_parser.sniff_columns` can propose the date column as the amount column.** On a header like `Transaction Date, Memo/Description, Amount (USD)` it proposes `amount ← Transaction Date`: that column scores 82 against the amount synonyms while the real `Amount (USD)` scores only 75, and nothing stops one column being claimed by two fields. Fails safe — `parse_transactions` returns 0 rows and the document is marked `failed`, so no wrong money reaches the database — but the user is shown a confident wrong answer with the correct column sitting unused, and the module's own docstring promises that an unmatched field is "simply absent from `mapping`". Fix: assign each column to at most one field, preferring the field it scores highest for. Found 2026-08-19 by running the uploader over real bank-export headers. | new |
